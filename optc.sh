...
if [ -n "$c1" ]; then
  adb shell ls /data/data/$PKG/files/Cache/GNPCACHE/*/*c1.png | tr '\r' ' ' | xargs -n1 -J % adb pull % $BASEDIR'/png/'$SUBDIR'/'
  exit 0
fi

if [ -n "$t1" ]; then
  adb shell ls /data/data/$PKG/files/Cache/GNPCACHE/*/*t1.png | tr '\r' ' ' | xargs -n1 -J % adb pull % $BASEDIR'/png/'$SUBDIR'/'
  exit 0
fi
